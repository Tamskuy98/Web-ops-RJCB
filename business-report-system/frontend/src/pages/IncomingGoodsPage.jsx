...
export default function IncomingGoodsPage() {
  ...
  const [form, setForm] = useState({ productId: '', quantity: '', supplier: '', date: '', note: '' });
  ...
  const openCreate = () => {
    setForm({ productId: '', quantity: '', supplier: '', date: new Date().toISOString().split('T')[0], note: '' });
    setModalOpen(true);
  };
                ...
                {paginated.map((item) => (
                  <tr key={item.id} className="border-t border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-center">
                      <button onClick={() => handleDelete(item.id)} className="p-1.5 hover:bg-red-50 text-red-600 rounded-lg"><Trash2 size={15} /></button>
                    </td>
                  </tr>
                ))}
                {paginated.length === 0 && <tr><td colSpan={6} className="py-8 text-center text-gray-400">No records found</td></tr>}
              </tbody>
            </table>
          </div>
          <div className="px-4 pb-3">
            <Pagination totalItems={items.length} itemsPerPage={perPage} currentPage={page} onPageChange={setPage} />
          </div>
        </div>
      )}

      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Add Incoming Goods">
