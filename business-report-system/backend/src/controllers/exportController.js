...
const { buildFinancialReportPdf } = require("../utils/financialPdfBuilder");
...
const exportSalesPDF = async (req, res, next) => {
  try {
    ...
    doc
      .fontSize(11)
      .text(`Total Sales: Rp ${totalSales.toLocaleString("id-ID")}`);
    ...
    sales.forEach((s) => {
      ...
      doc.text(new Date(s.date).toISOString().split("T")[0], cols[0], y, {
        width: 75,
      });
      doc.text(s.product.name.substring(0, 22), cols[1], y, { width: 155 });
      doc.text(String(s.quantity), cols[2], y, { width: 55 });
      doc.text(Number(s.priceSell).toLocaleString("id-ID"), cols[3], y, {
        width: 65,
      });
      doc.text(Number(s.total).toLocaleString("id-ID"), cols[4], y, {
        width: 65,
      });
      doc.text(Number(s.profit).toLocaleString("id-ID"), cols[5], y, {
        width: 65,
      });
      y += 15;
    });
...
/**
 * Generates the financial report PDF for the Dashboard's active date filter.
 * Controller stays thin: data gathering lives in financialReportService,
 * PDF rendering lives in utils/financialPdfBuilder.
 */
const exportFinancialReportPDF = async (req, res, next) => {
  try {
    ...
    if ((startDate && !endDate) || (!startDate && endDate)) {
      const error = new Error("startDate dan endDate harus diisi bersamaan.");
      error.statusCode = 400;
      throw error;
    }

    const reportData = await financialReportService.getFinancialReportData({
      startDate,
      endDate,
    });
    buildFinancialReportPdf(res, reportData);
  } catch (error) {
    next(error);
  }
};
